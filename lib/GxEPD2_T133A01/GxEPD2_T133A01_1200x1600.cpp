...
//
// Vendored into the TRMNL firmware lib/ folder.  Diagnostic output is routed
// through the Arduino Serial object (USB CDC) instead of the example's Serial0
// so it works with this project's logging setup.
...
// Diagnostic stream.  The upstream example used Serial0 (UART0); here we use
// the default Serial so messages show up alongside the firmware's own logs.
#ifndef GxEPD2_T133A01_DBG
#define DBG Serial
#else
#define DBG GxEPD2_T133A01_DBG
#endif
                                      ...
                                      bool pulldown_rst_mode)
{
  ...
  _diag_enabled = (serial_diag_bitrate > 0);
